...
![AIGraphUI](./frontend/public/gen_ui_diagram.png)

## Overview

This application aims to provide a template for building generative UI applications with AIGraphUI.
It comes pre-built with a few UI features which you can use to play about with AIGraphUI. The UI components are built using [Shadcn](https://ui.shadcn.com/).
...
```bash
git clone https://github.com/bracesproul/aigraphui.git
...
- AIGraphUI outside of the chatbot window: Have the UI dynamically render in different areas on the screen. E.g a dashboard, where the components are dynamically rendered based on the LLMs output.
