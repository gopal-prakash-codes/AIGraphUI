...
class AIGraphUIState(TypedDict, total=False):
    input: HumanMessage
    result: Optional[str]
...
def invoke_model(state: AIGraphUIState, config: RunnableConfig) -> AIGraphUIState:
...
def invoke_tools_or_return(state: AIGraphUIState) -> str:
...
def invoke_tools(state: AIGraphUIState) -> AIGraphUIState:
    tools_map = {
...
def create_graph() -> CompiledGraph:
    workflow = StateGraph(AIGraphUIState)
